...
source('/scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/LikelihoodFun/GL_maineqs.R')

#Read in the pre-GI streamflow in cfs units----
#Already has desired date range
Qp = read.table(paste0('/scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/obs/Q', arg[2], '.txt'), header=TRUE, sep='\t', stringsAsFactors=FALSE)
...
world = read.csv('/scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/RHESSys_Baisman30m_g74/worldfiles/worldfile.csv', stringsAsFactors = FALSE)
res = 30
...
#Flow matrix
Qflow = matrix(0, nrow = nrow(Qp), ncol = as.numeric(arg[9]))
#Objective vectors
