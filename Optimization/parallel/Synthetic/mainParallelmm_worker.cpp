...
void model_wrapper(double *vars, double *objs, double *constr, int *evals, int *isle, int *worker){
    ...
    sprintf(command, "sh /scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/RunArray_NFT_GI_worker.sh %u %u %u %u &>> %s", N, i, inum, work, fc);
    std::system(command);
    
    // Move output from system command to storage folder
    char fdir[256];
    sprintf(fdir, "/scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/RHESSysRuns/output/%s", fc);
...
int main(int argc, char* argv[]) {
    ...
    sprintf(outputFilename, "/scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/RHESSysRuns/output/GI_S%d.set", seed); // output path (make sure this exists)
    sprintf(runtime, "/scratch/js4yd/Bais910Hill30mOptimizeGI_Syn/RHESSysRuns/output/GI_S%d_M%%d.runtime", seed); // runtime path (make sure this exists)
