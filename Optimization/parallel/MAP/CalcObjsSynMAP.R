...
source('/nv/vol288/quinnlab-value/BaisOptMAP/Bais910Hill30mOptimizeGI_MAP/LikelihoodFun/GL_maineqsMAP.R')

#Read in the pre-GI streamflow in cfs units----
#Already has desired date range
Qp = read.table(paste0('/nv/vol288/quinnlab-value/BaisOptMAP/Bais910Hill30mOptimizeGI_MAP/obs/Q', arg[2], '.txt'), header=TRUE, sep='\t', stringsAsFactors=FALSE)
...
world = read.csv('/nv/vol288/quinnlab-value/BaisOptMAP/Bais910Hill30mOptimizeGI_MAP/RHESSys_Baisman30m_g74/worldfiles/worldfile.csv', stringsAsFactors = FALSE)
res = 30
...
Q = vroom(paste0(arg[4], '/Run', arg[1], '_MAPAllSyn_basin.daily'), delim = ' ', col_names = TRUE, col_types = cols(.default=col_double()), progress = FALSE)
...
write.table(Objs, paste0(arg[4], '/Run', arg[1], '_MAPAllSyn_Objs.txt'), row.names = FALSE, col.names = FALSE, sep = '\n')
