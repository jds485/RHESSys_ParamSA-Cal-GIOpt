...
fs = grep(x=list.files(), pattern='_MAPAllSyn', value=TRUE)
...
for (num in 1:length(fs)){
  ...
  #read in objectives
  DVOi = t(read.table(paste0(getwd(), '/', fs[num], arg[2], '/output/Run', n, '_MAPAllSyn_Objs.txt'), sep='\t'))
...
#Save compiled data
setwd(arg[3])
write.table(DVO, paste0('DVO_c_MAPSyn.txt'), row.names=FALSE, col.names=TRUE, sep='\t')