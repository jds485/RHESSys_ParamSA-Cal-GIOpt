...
for (num in 2:length(fs)){
  ...
  #some runs did not finish, so need to make those have NA objectives
  if(length(grep(list.files(paste0(getwd(), '/', fs[num], arg[2], '/output')), pattern = 'Objs.txt')) == 0){
    NAvec = as.data.frame(x = list(Flood = NA, LowFlow = NA, NumTrees = NA))
    DVOi = cbind(DVOi, NAvec)
...
#Save compiled data
setwd(arg[3])
write.table(DVO, paste0('DVO_c_MOROMinMax.txt'), row.names=FALSE, col.names=TRUE, sep='\t')