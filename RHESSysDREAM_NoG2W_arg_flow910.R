...
#3 - Observed streamflow record path (.txt file, tab separated)
...
#9 - File with chain starting locations (.csv file with headers)
...
startValues = as.matrix(read.csv(file = arg[9], stringsAsFactors = FALSE))
...
settings_Parext = list(iterations = as.numeric(arg[11]), #Need at least one more iteration than the burnin to report a value to the output chain
                        gamma= NULL, #Not used in code
                        eps = as.numeric(arg[12]), #Not adding normal noise to the proposal update.
                        e = as.numeric(arg[13]), #Multiplying gamma by 1.05 in proposal update 
                        parallel = NULL, #Specified in setup
                        Z = NULL, #Computed in code. Supply upon manual restart.
                        ZupdateFrequency = as.numeric(arg[14]), #Z is not updated until after burnin if the update frequency is greater than 1. Making equal to updateInterval.
                        pSnooker = as.numeric(arg[15]), #10% chance of a snooker update
                        DEpairs = as.numeric(arg[16]), #Number of chains used to compute the DE proposal update. In papers, randomly choose 1, 2, or 3 to use.
                        nCR = as.numeric(arg[17]), #Update about 12 variables each time. nCR >=2 recommended in papers.
                        pCRupdate = TRUE, #Use crossover updates
                        updateInterval = as.numeric(arg[18]), #Must be greater than or equal to nCR. Best to be smaller.
                        burnin = as.numeric(arg[19]), #burnin must be greater than or equal to adaptation, or all of the adaptation steps must be manually removed before summarizing chains.
                        adaptation = as.numeric(arg[20]), #Adaptation should be about 20 % of the total NFE. Doing this manually.
                        thin = as.numeric(arg[21]), #Thin = 1 to allow for post-solver manual thinning
...
list.save(out_Parext$chain, paste0('OutputChains', arg[30], '.yaml'))
