...
for (i in 1:(ncol(BasinSF)-1)){
  ...
  HillTNMed[,ColInd_h+2] = as.numeric(scan(file = paste0(getwd(), '/', fs_h[i]), sep = '\t', what = 'numeric', blank.lines.skip = TRUE, quiet = TRUE, nlines = 1, skip = 1))
}
...
#Save R data file
setwd('/scratch/js4yd/MorrisSA/TNprocessing/')
save.image(file = "TNSAreps_HillMed_All.RData", safe = FALSE)

#tables
write.table(round(HillTNMed,3), file = 'SAResults_HillTNMed_p3_All.txt', row.names = FALSE, sep = '\t')
