#!/bin/bash
#SBATCH -D /scratch/js4yd/GI_RandomSeedEval/RHESSysRuns/
#SBATCH -o /scratch/js4yd/GI_RandomSeedEval/RHESSysRuns/output/Run_%a.out
#SBATCH --ntasks=1                              # Number of tasks per serial job (must be 1)
#SBATCH -p standard           									# Queue partition name "standard"
#SBATCH -A quinnlab       											# allocation name
#SBATCH -t 01:30:00       											# Run time per serial job (hh:mm:ss) - up to 36 hours
...
BASEDIR='/scratch/js4yd/GI_RandomSeedEval'
...
cp "$BASEDIR"/GIS2RHESSys/soilCollection_Cal.csv ./GIS2RHESSys
cp "$BASEDIR"/GIS2RHESSys/vegCollection_modified_Opt.csv ./GIS2RHESSys
...
singularity exec "$SINGIMAGE"/rhessys_v3.img python "$BASEDIR"/RHESSysRuns/ModifyVeg.py "$BASEDIR"/RHESSysRuns/Run"$i"/GIS2RHESSys "$PROJDIR"/"$RHESSysNAME" "$PROJDIR"/"$RHESSysNAME"/defs 'vegCollection_modified_Opt.csv'
...
mkdir ./GIAllocation
cp "$BASEDIR"/GIAllocation/AllocateGI_Rivanna.R ./GIAllocation
...
singularity exec "$SINGIMAGE"/rhessys_v3.img Rscript "$PROJDIR"/GIAllocation/AllocateGI_Rivanna.R "$i" 'MaxGI30m.csv' 'lulcFrac30m.csv' "$PROJDIR"/"$RHESSysNAME" '6' '3' 'lulcFrac30m_GI.csv' '9' '900'
...
cd "$PROJDIR"
singularity exec "$SINGIMAGE"/rhessys_v3.img grass74 "$LOCATION"/$MAPSET --exec Rscript "$GITHUBLIBRARIES"/g2w_cf_RHESSysEC.R "$PROJDIR" "$PROJDIR"/"$RHESSysNAME"/vegCollection_modified_Opt.csv "$PROJDIR"/GIS2RHESSys/soilCollection_Cal.csv "$PROJDIR"/GIS2RHESSys/lulcCollectionEC_Cal.csv "$templateFile" "$RLIBPATH"
...
rm "$PROJDIR"/"$RHESSysNAME"/MaxGI30m.csv
...
rm -r "$PROJDIR"/"$RHESSysNAME"/defs
#Remove some un-needed output
rm "$PROJDIR"/"$RHESSysNAME"/output/Run"$i"_basin.hourly
rm "$PROJDIR"/"$RHESSysNAME"/output/Run"$i"_basin.monthly
rm "$PROJDIR"/"$RHESSysNAME"/output/Run"$i"_basin.yearly
rm "$PROJDIR"/"$RHESSysNAME"/output/Run"$i"_hillslope.hourly
rm "$PROJDIR"/"$RHESSysNAME"/output/Run"$i"_hillslope.monthly
rm "$PROJDIR"/"$RHESSysNAME"/output/Run"$i"_hillslope.yearly