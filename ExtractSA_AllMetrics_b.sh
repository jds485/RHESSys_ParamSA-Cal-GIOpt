...
#SBATCH -o /scratch/js4yd/MorrisSA/SAmetrics/output/Run_1b%a.out
#SBATCH --ntasks=1                              # Number of tasks per serial job (must be 1)
#SBATCH -p standard           									# Queue partition name "standard"
#SBATCH -A quinnlab       											# allocation name
#SBATCH -t 00:07:00       											# Run time per serial job (hh:mm:ss) - up to 36 hours
...
Rscript "$INDIR"/ExtractSAtrajectories_AllMetrics_b.R "$SLURM_ARRAY_TASK_ID" "$OUTDIR" "/scratch/js4yd/MorrisSA/SAmetrics/EEs_All_Setup_paperb.RData"
