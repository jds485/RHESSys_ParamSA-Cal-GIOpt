...
#%% Read the Chains_df and problem file with bounds of parameters
ProbFile = pd.read_csv(sys.argv[5])
Chains_df = pd.read_csv(sys.argv[1])
...
basin = pd.read_csv(sys.argv[3]+'/basin_basin.def', delim_whitespace=True, header=None, dtype = 'object')
zone = pd.read_csv(sys.argv[3]+'/zone_zone.def', delim_whitespace=True, header=None, dtype = 'object')
hill = pd.read_csv(sys.argv[3]+'/hillslope_hillslope.def', delim_whitespace=True, header=None, dtype = 'object')
...
for i in [int(sys.argv[8]) - 1]:
    ...
    #Loop over the Chains_df data for z_. These variables were random and should be changed
    IndZ = []
    for z in range(len(Chains_df.columns)):
        #If the z is in the first position, it's a zone file. Same concept used for other defs below.
        if string.find(Chains_df.columns[z], 'z') == 0:
    ...
    for z in range(len(IndZ)):
        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndZ[z]]].values[0]
        ...
        zc.iloc[:,0][zc.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndZ[z]].round(roundTol)
    ...
    #Loop over the Chains_df data for h_. These variables were random and should be changed
    IndH = []
    for h in range(len(Chains_df.columns)):
        if string.find(Chains_df.columns[h], 'h') == 0:
            #Record the index. This variable should be used for hillslopes
            IndH.append(h)
    del h
    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
    for h in range(len(IndH)):
        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndH[h]]].values[0]
        #Find the defName in the def file and replace it with the ith Morris row
        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
        hc.iloc[:,0][hc.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndH[h]].round(roundTol)
...
#    #Loop over the Chains_df data for l_. These variables were random and should be changed
#    IndL1 = []
#    for l in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[l], 'l1') == 0:
#            #Record the index. This variable should be used for land_grass
#            IndL1.append(l)
#    del l
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for l in range(len(IndL1)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndL1[l]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        l1c.iloc[:,0][l1c.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndL1[l]].round(roundTol)
#    
#    #Fixme: see above for guarantee of columns in same order
...
#    #Write new land_grass file
    land_grass.to_csv('landuse_grass.def', sep=' ', index=False, header=False, encoding='utf-8')
...
#    IndL2 = []
#    for l in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[l], 'l2') == 0:
#            #Record the index. This variable should be used
#            IndL2.append(l)
#    del l
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for l in range(len(IndL2)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndL2[l]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        l2c.iloc[:,0][l2c.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndL2[l]].round(roundTol)
#
#    #Fixme: see above for guarantee of columns in same order
...
#    
#    #Write new file
    land_undev.to_csv('landuse_undeveloped.def', sep=' ', index=False, header=False, encoding='utf-8')
...
#    IndL3 = []
#    for l in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[l], 'l3') == 0:
#            #Record the index. This variable should be used
#            IndL3.append(l)
#    del l
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for l in range(len(IndL3)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndL3[l]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        l3c.iloc[:,0][l3c.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndL3[l]].round(roundTol)
#
#    #Fixme: see above for guarantee of columns in same order
...
#
#    #Write new file
    land_urban.to_csv('landuse_urban.def', sep=' ', index=False, header=False, encoding='utf-8')
#    del l3c, IndL3, defName, l, ck, checkVals
    ...
    IndL4 = []
    for l in range(len(Chains_df.columns)):
        if string.find(Chains_df.columns[l], 'l4') == 0:
            #Record the index. This variable should be used
            IndL4.append(l)
    del l
    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
    for l in range(len(IndL4)):
        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndL4[l]]].values[0]
        #Find the defName in the def file and replace it with the ith Morris row
        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
        l4c.iloc[:,0][l4c.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndL4[l]].round(roundTol)
    ...
    #Loop over the Chains_df data for l_. These variables were random and should be changed
    IndS9 = []
    for s in range(len(Chains_df.columns)):
        if string.find(Chains_df.columns[s], 's9') == 0:
            #Record the index. This variable should be used
            IndS9.append(s)
    del s
    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
    for s in range(len(IndS9)):
        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[IndS9[s]]].values[0]
        #Find the defName in the def file and replace it with the ith Morris row
        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
        s9.iloc[:,0][s9.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,IndS9[s]].round(roundTol)
    ...
    Inds109 = []
    for s in range(len(Chains_df.columns)):
        if string.find(Chains_df.columns[s], 's109') == 0:
            #Record the index. This variable should be used
            Inds109.append(s)
    del s
    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
    for s in range(len(Inds109)):
        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[Inds109[s]]].values[0]
        #Find the defName in the def file and replace it with the ith Morris row
        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
        s109.iloc[:,0][s109.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,Inds109[s]].round(roundTol)
...
#    Inds8 = []
#    for s in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[s], 's8') == 0:
#            #Record the index. This variable should be used
#            Inds8.append(s)
#    del s
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for s in range(len(Inds8)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[Inds8[s]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        s8.iloc[:,0][s8.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,Inds8[s]].round(roundTol)
#
#    #Sorting alphabetically to check columns are the same
...
#    s8.to_csv('soil_silt_loam.def', sep=' ', index=False, header=False, encoding='utf-8')
    soil_sloam.to_csv('soil_silt_loam.def', sep=' ', index=False, header=False, encoding='utf-8')
...
#    Inds108 = []
#    for s in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[s], 's108') == 0:
#            #Record the index. This variable should be used
#            Inds108.append(s)
#    del s
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for s in range(len(Inds108)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[Inds108[s]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        s108.iloc[:,0][s108.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,Inds108[s]].round(roundTol)
#
#    #Sorting alphabetically to check columns are the same
...
#    s108.to_csv('soil_silt_loam_compact.def', sep=' ', index=False, header=False, encoding='utf-8')
    soil_csloam.to_csv('soil_silt_loam_compact.def', sep=' ', index=False, header=False, encoding='utf-8')
#    del s108, Inds108, defName, s, ck, checkVals, checkVals2
    ...
    #Loop over the Chains_df data for l_. These variables were random and should be changed
    Indv102 = []
    for v in range(len(Chains_df.columns)):
        if string.find(Chains_df.columns[v], 'v102') == 0:
            #Record the index. This variable should be used
            Indv102.append(v)
    del v
    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
    for v in range(len(Indv102)):
        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[Indv102[v]]].values[0]
        #Find the defName in the def file and replace it with the ith Morris row
        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
        v102.iloc[:,0][v102.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,Indv102[v]].round(roundTol)
...
#    Indv3 = []
#    for v in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[v], 'v3') == 0:
#            #Record the index. This variable should be used
#            Indv3.append(v)
#    del v
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for v in range(len(Indv3)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[Indv3[v]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        v3.iloc[:,0][v3.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,Indv3[v]].round(roundTol)
#
#    #Sorting alphabetically to check columns are the same
...
#    v3.to_csv('stratum_grass.def', sep=' ', index=False, header=False, encoding='utf-8')
    veg_grass.to_csv('stratum_grass.def', sep=' ', index=False, header=False, encoding='utf-8')
...
#    v4 = veg_NonVeg.copy(deep=True)
    #Loop over the Chains_df data for l_. These variables were random and should be changed
    #Fixme: not in calibration parameters
#    Indv4 = []
#    for v in range(len(Chains_df.columns)):
#        if string.find(Chains_df.columns[v], 'v4') == 0:
#            #Record the index. This variable should be used
#            Indv4.append(v)
#    del v
#    #Now loop over these columns and extract the ProbFile variable name that matches the def file name
#    for v in range(len(Indv4)):
#        defName = ProbFile.iloc[:,1][ProbFile.iloc[:,0] == Chains_df.columns[Indv4[v]]].values[0]
#        #Find the defName in the def file and replace it with the ith Morris row
#        #Fixme: reporting roundTol decimal places here to avoid scientific notation, and avoid loss of precision in atm trans lapse rate
#        v4.iloc[:,0][v4.iloc[:,1] == defName] = '%.10f' % Chains_df.iloc[i,Indv4[v]].round(roundTol)
#
#    #Sorting alphabetically to check columns are the same
...
#    
#    #Write new file
    veg_NonVeg.to_csv('stratum_nonveg.def', sep=' ', index=False, header=False, encoding='utf-8')
