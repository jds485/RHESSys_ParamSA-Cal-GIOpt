...
SimQ = pd.read_csv('SAResults_BasinStreamflow_p4_Reordered_Add5_Likes_pre.txt',delimiter='\t')
SimQ = SimQ.iloc[1782:]
...
for i in range(start,stop):
    comparedata = np.array(SimQ['Replicate' + str(ID[i])])
    
    #Compute the weekly, monthly, and annual timeseries from the daily timeseries
    cSimQ['mFlow'] = cSimQ['Replicate' + str(ID[i])].resample('M').sum()
    cSimQ['aFlow'] = cSimQ['Replicate' + str(ID[i])].resample('A').sum()
    ...
    for k in range(len(cTrueQ['Flow'])):
        if np.mod(k+1, 7) == 0:
            ...
            wSimQ[w] = cSimQ['Replicate' + str(ID[i])].iloc[(k-6):k+1].sum()
            w = w + 1
    
    #Compute metrics to append to dataframe:
    #SSE
    SSEd = np.sum((cTrueQ['Flow'] - cSimQ['Replicate' + str(ID[i])])**2)
    ...
    #LNSE
    LNSEd = 1. - np.sum((np.log(cTrueQ['Flow']) - np.log(cSimQ['Replicate' + str(ID[i])]))**2)/np.sum((np.log(cTrueQ['Flow']) - np.mean(np.log(cTrueQ['Flow'])))**2)
    ...
    #Percent Bias
    pBias = 100. * (np.sum(cSimQ['Replicate' + str(ID[i])] - cTrueQ['Flow']) / np.sum(cTrueQ['Flow']))
