...
#SBATCH -o /scratch/js4yd/MorrisSA/RHESSysRuns/output/Run_%a.out
...
#This shell script is the same as RunArray, but does not move files to a permanent storage at the end and deletes existing folders.
...
if [ -d ""$BASEDIR"/RHESSysRuns/Run"$i"" ] 
then
    rm -r ""$BASEDIR"/RHESSysRuns/Run"$i"" 
fi

#Also remove the value storage directory if it exists
if [ -d ""$OUTDIR"/Run"$i"" ] 
then
    rm -r ""$OUTDIR"/Run"$i"" 
fi
